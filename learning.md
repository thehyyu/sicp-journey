...
- Zoom Out：告訴 AI「我要開始 Ch.X」，AI 中文說明這章全景。
- Zoom In：逐節推進，告訴 AI 節號 → AI 中文講解 → 你跑程式做習題 → 貼解法討論。
- 測試：能說清楚「它在什麼情況下會壞掉」，代表真的理解了。

**不需要讀英文原文。** 母語是華語，AI 就是你的中文版 SICP。
...
- **1.1.7** 遞迴 = 函數呼叫自己。一定要有停止條件，沒有出口就是 stack overflow。牛頓法示範了「計算過程」和「數學定義」的差別——定義說「什麼是答案」，過程說「怎麼找到答案」。
- **1.1.4** 自己定義的程序和內建的沒有差別，定義好之後就能像 `+`、`*` 一樣直接組合使用。
- **1.1.3** 括號就是結構。`(+ 1 2)` 不只是語法，它讓程式碼本身就是一棵樹。解釋器不需要解析複雜的語法規則，括號已經把結構說清楚了。這個特性叫 homoiconicity——程式碼和資料用同一種形式表示。Ch.4 寫解釋器時會深刻感受到這有多方便。
